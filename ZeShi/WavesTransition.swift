...
//        carryInView.center = CGPoint(x: carryViewOriginalCenter.x, y: -carryInView.bounds.height / 2)
        ...
        halfway = {
            ...
            self.carryInView.center.y = self.carryInView.bounds.height / 2
            UIViewPropertyAnimator.runningPropertyAnimator(withDuration: 1.2, delay: 1.6, options: .curveEaseInOut, animations: {
        ...
        let yrange = ClosedRange(uncheckedBounds: (50, containerView.bounds.height - 20))
        var emojis = emojiChoices
        var xpos: CGFloat = 50 + CGFloat(50.arc4random)
        while xpos < containerView.bounds.width - 50 {
            ...
            let label = makeEmojiLabel(_: String(emoji), at: CGPoint(x: xpos, y: CGFloat.random(in: yrange)))
            label.isHidden = true
            emojiViews.append(label)
            xpos += CGFloat(50.arc4random + 50)
            if emojiChoices.isEmpty { break }
        }
