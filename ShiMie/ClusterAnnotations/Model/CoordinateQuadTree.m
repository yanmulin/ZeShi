...
@interface CoordinateQuadTree ()

@property (nonatomic, assign) BoundingBox maxBounding;
...
- (void)buildTreeWithPOIs:(NSArray *)pois
{
    ...
    self.maxBounding = quadTreeNodeDataArrayForPOIs(dataArray, pois);
    ...
    NSLog(@"build tree.");
    /*建立四叉树索引. */
    self.root = QuadTreeBuildWithData(dataArray, [pois count], self.maxBounding, 4);
...
- (void)insertPoi:(AMapPOI *)poi {
    ...
    QuadTreeNodeData data = QuadTreeNodeDataForAMapPOI(poi);
    if (!BoundingBoxContainsData(self.maxBounding, data)) {
        @throw @"insert a poi outside maxBounding into CoordinateQuadTree";
    }
...
- (instancetype)initWith:(BoundingBox)boundingBox maxCount:(int)count
{
    self = [super init];
    if (self) {
        self.maxBounding = boundingBox;
        self.maxCount = count;
        self.count = 0;
    }
    return self;
...
- (void)clean
{
    ...
    {
        NSLog(@"free tree.");
        self.root = FreeQuadTreeNode(self.root);
    }
    self.count = 0;
}
